...
from common import (
    ...
    DEFAULT_ACTOR_CSV_PATH,
    DEFAULT_MODEL_PATH,
    blendshapes_to_dict,
    build_face_landmarker,
    calculate_actor_emotions,
    draw_emotion_box,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze an actor reference video with the latest HCI-acting-coach pipeline.")
    parser.add_argument("video_path", type=Path, help="Path to the actor reference video file.")
    parser.add_argument("--model-path", type=Path, default=DEFAULT_MODEL_PATH, help="Path to face_landmarker.task.")
    parser.add_argument("--output-csv", type=Path, default=DEFAULT_ACTOR_CSV_PATH, help="Path to save actor_expression.csv.")
...
def main() -> int:
    ...
    print("Actor video opened. Starting analysis.")
    ...
    while True:
        ...
        if frame_idx % args.sample_every == 0:
            ...
            if detection_result.face_landmarks and detection_result.face_blendshapes:
                ...
                data.update(blendshapes_to_dict(detection_result.face_blendshapes[0]))
                data = calculate_actor_emotions(data)
        ...
        cv2.imshow("HCI Acting Coach Actor Analysis", frame)
    ...
    if not results:
        print("No face data was saved. Check that the actor face is visible in the video.")
        return 1

    args.output_csv.parent.mkdir(parents=True, exist_ok=True)
    df = pd.DataFrame(results)
    df.to_csv(args.output_csv, index=False, encoding="utf-8-sig")
    print(f"Saved actor CSV to {args.output_csv}")
